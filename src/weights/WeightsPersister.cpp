...
STATIC void WeightsPersister::persistWeights( std::string filepath, std::string trainingConfigString, NeuralNet *net, int epoch, int batch, float annealedLearningRate, int numRight, float loss, float normalizationTranslate, float normalizationScale ) { // we should probably rename 'weights' to 'model' now that we are storing normalization data too?
    ...
    persistArrayInts[1] = 2; // data file version number
    ...
    persistArrayFloats[7] = normalizationTranslate;
    persistArrayFloats[8] = normalizationScale;
    strcpy_safe( persistArray + 9 * 4, trainingConfigString.c_str(), 800 );
...
STATIC bool WeightsPersister::loadWeights( std::string filepath, std::string trainingConfigString, NeuralNet *net, int *p_epoch, int *p_batch, float *p_annealedLearningRate, int *p_numRight, float *p_loss ) {
    float translation;
    float scale;
    return loadWeights( filepath, trainingConfigString, net, p_epoch, p_batch, p_annealedLearningRate, p_numRight, p_loss, &translation, &scale, 1 );
}
STATIC bool WeightsPersister::loadWeights( std::string filepath, std::string trainingConfigString, NeuralNet *net, int *p_epoch, int *p_batch, float *p_annealedLearningRate, int *p_numRight, float *p_loss, float *p_translation, float *p_scale ) {
    return loadWeights( filepath, trainingConfigString, net, p_epoch, p_batch, p_annealedLearningRate, p_numRight, p_loss, p_translation, p_scale, 2 );
}
STATIC bool WeightsPersister::loadWeights( std::string filepath, std::string trainingConfigString, NeuralNet *net, int *p_epoch, int *p_batch, float *p_annealedLearningRate, int *p_numRight, float *p_loss, float *p_translation, float *p_scale, int minimumVersion ) {
    if( FileHelper::exists( filepath ) ){
        int headerSize = 1024;
        long fileSize;
        char * data = FileHelper::readBinary( filepath, &fileSize );

        if( !checkData( data, headerSize, fileSize, minimumVersion ) ){
            delete [] data;
            return false;
        }
        ...
        if( version == 1 ) {
            *p_translation = 0.0f;  // since v1 doesnt contain these...
            *p_scale = 1.0f;
            return loadWeightsv1( data, fileSize, trainingConfigString, net, p_epoch, p_batch, p_annealedLearningRate, p_numRight, p_loss );
        } else if( version == 2 ) {
            return loadWeightsv2( data, fileSize, trainingConfigString, net, p_epoch, p_batch, p_annealedLearningRate, p_numRight, p_loss, p_translation, p_scale );
        } else {
...
STATIC bool WeightsPersister::loadWeightsv1( char *data, long fileSize, std::string trainingConfigString, NeuralNet *net, int *p_epoch, int *p_batch, float *p_annealedLearningRate, int *p_numRight, float *p_loss ) {
        int headerSize = 1024;
...
STATIC bool WeightsPersister::loadWeightsv2( char *data, long fileSize, std::string trainingConfigString, NeuralNet *net, int *p_epoch, int *p_batch, float *p_annealedLearningRate, int *p_numRight, float *p_loss, float *p_translation, float *p_scale ) {
    int headerSize = 1024;
    data[headerSize - 1] = 0; // null-terminate the string, if not already done

    if( trainingConfigString != std::string(data + 9 * 4) ) {
        std::cout << "training options dont match weights file" << std::endl;
        std::cout << "in file: [" + std::string( data + 9 * 4 ) + "]" << std::endl;
        std::cout << "current options: [" + trainingConfigString + "]" << std::endl;
...
//    std::cout << "read weights from file "  << (fileSize/1024) << "KB" << std::endl;
    ...
    int version = dataAsInts[1];
    if( version != 2 ) {
        throw std::runtime_error( "unexpected version " + toString( version ) );
    }        
    *p_epoch = dataAsInts[2];
    *p_batch = dataAsInts[3];
    *p_numRight = dataAsInts[4];
    *p_loss = dataAsFloats[5];
    *p_annealedLearningRate = dataAsFloats[6];
    *p_translation = dataAsFloats[7];
    *p_scale = dataAsFloats[8];
    copyArrayToNetWeights( allWeightsArray, net );

    delete [] data;
    return true;
}
STATIC bool WeightsPersister::checkData( const char * data, long headerSize, long fileSize, int minimumVersion ) {
    ...
    if( dataAsInts[1] != 1 && dataAsInts[1] != 2 ) {
        std::cout << "weights file version not known" << std::endl;
        return false;
    }

    if( dataAsInts[1] < minimumVersion ) {
        std::cout << "weights file version too low" << std::endl;
        return false;
    }

    return true;
}
STATIC bool WeightsPersister::loadConfigString( std::string filepath, std::string & configString, float *p_translate, float *p_scale ) {
    if( FileHelper::exists( filepath ) ){
        int headerSize = 1024;
        long fileSize;
        char * data = FileHelper::readBinary( filepath, &fileSize );

        if( !checkData( data, headerSize, fileSize, 2 ) ) {
            delete [] data;
            return false;
        }
        ...
        int version = dataAsInts[1];
        if( version == 1 ) {
            throw std::runtime_error( "version too low " + toString(version) );
        } else if( version == 2 ) {
            configString = std::string( data + 9 * 4 + 7 );
            *p_translate = dataAsFloats[7];
            *p_scale = dataAsFloats[8];
        } else {
