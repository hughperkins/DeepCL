...
VIRTUAL void BackpropErrorsv2Naive::backpropErrors( int batchSize, 
    ...
    broadcastMultiply->in( batchSize * dim.inputCubeSize )->in( errorsForUpstreamWrapper )->in( inputDataWrapper );
    broadcastMultiply->run_1d(globalSize, workgroupsize);
    cl->finish();
    StatefulTimer::instance()->timeCheck("BackpropErrorsv2Naive after broadcastmultiply" );
...
BackpropErrorsv2Naive::BackpropErrorsv2Naive( OpenCLHelper *cl, LayerDimensions dim, ActivationFunction const *upstreamFn ) :
    ...
    // generated using cog:
    const char * broadcastMultiplySource =  
    "// Copyright Hugh Perkins 2014 hughperkins at gmail\n" 
    "//\n" 
    "// This Source Code Form is subject to the terms of the Mozilla Public License,\n" 
    "// v. 2.0. If a copy of the MPL was not distributed with this file, You can\n" 
    ...
    "\n" 
    "";
    broadcastMultiply = cl->buildKernelFromString( broadcastMultiplySource, "broadcast_multiply", options, "cl/backproperrorsv2.cl" );
    // [[[end]]]
