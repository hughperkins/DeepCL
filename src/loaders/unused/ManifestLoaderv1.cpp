...
VIRTUAL bool ManifestLoaderv1::isFormatFor( char *header1024 ) {
    char const*header = "# format: imagenetv1";
    int headerLen = strlen( header );
    char *typecopy = new char[headerLen + 1];
    strncpy( typecopy, header1024, headerLen );
    typecopy[headerLen] = 0;
    return string( typecopy ) == string( header );
}
VIRTUAL void ManifestLoaderv1::getDimensions( std::string filepath, int *p_N, int *p_numPlanes, int *p_imageSize ) {
...
VIRTUAL void ManifestLoaderv1::load( std::string filepath, unsigned char *data, int *labels, int startRecord, int numRecords ) {
    int N;
    int imageSize;
    int numPlanes;
//    int imagesSize;
