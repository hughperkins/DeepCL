...
public:
    static void randomize( std::mt19937 &random, float *values, int numValues, float minvalue = -0.1f, float maxvalue = 0.1f ) {
        for( int i = 0; i < numValues; i++ ) {
            values[i] = random() / (float)std::mt19937::max() * (maxvalue-minvalue) - maxvalue;
        }
    }
    static std::mt19937 randomize( float *values, int numValues, float minvalue = -0.1f, float maxvalue = 0.1f ) {
        std::mt19937 random;
